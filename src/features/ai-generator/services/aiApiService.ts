 ...
 */
export const transformToTemplate = (result: any, usageData?: TokenUsageInfo): AiGenerationResult => {
    ...
    const inflatedColumns = result.columns.map((col: any) => {
        ...
        if (typeof finalFormula === 'string') {
            const multiMatch = finalFormula.match(/[×\*xX]\s*\(?(-?\d+(\.\d+)?)\)?/);
            if (multiMatch) {
...
export const callAiScoreboardApi = async (
    ...
    try {
        ...
        if (!response.ok) {
            ...
            const diagnosticInfo = JSON.stringify({ raw: `HTTP ${response.status} Error`, error: errorText });
        ...
        return transformToTemplate(result, usageData);
