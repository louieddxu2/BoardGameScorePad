...
import { integrationTranslations, IntegrationTranslationKey } from '../../../i18n/integration';

// Static translation helper for non-React context
const t = (key: IntegrationTranslationKey, params?: Record<string, string | number>): string => {
    const lang = (localStorage.getItem('app_language') || 'zh-TW') as 'zh-TW' | 'en';
    const dict = (integrationTranslations[lang] || integrationTranslations['zh-TW']) as any;
...
export const bggImportService = {
    ...
    async analyzeData(csvText: string, onProgress?: (msg: string) => void): Promise<ImportAnalysisReport> {
        ...
        if (csvText.trim().toLowerCase().startsWith('<!doctype html') || csvText.trim().toLowerCase().startsWith('<html')) {
            throw new Error(t('bgg_err_html_detected'));
        }

        const rows = parseCSV(csvText);
        if (rows.length < 2) throw new Error(t('bgg_err_empty_csv'));
        ...
        if (headerRowIndex >= rows.length) throw new Error(t('bgg_err_no_header'));
        ...
        if (idx.id === -1 || idx.name === -1) {
            throw new Error(t('bgg_err_format_mismatch'));
        }

        if (onProgress) onProgress(t('bgg_prog_parsing', { count: rows.length - 1 }));
    ...
    async importData(
        ...
        if (onProgress) onProgress(t('bgg_prog_preparing', { count: sourceGames.length }));
        ...
        for (let i = 0; i < totalBgg; i += CHUNK_SIZE) {
            ...
            if (onProgress) onProgress(t('bgg_prog_writing', { current: currentProgress, total: totalBgg }));
        ...
        if (savedGameUpdates.length > 0) {
            if (onProgress) onProgress(t('bgg_prog_updating_links', { count: savedGameUpdates.length }));
        ...
        if (onProgress) onProgress(t('bgg_prog_propagating'));
