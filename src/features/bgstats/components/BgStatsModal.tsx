...
import { useBgStatsTranslation } from '../../../i18n/bgstats';
...
const BgStatsModal: React.FC<BgStatsModalProps> = ({ isOpen, onClose, onImport }) => {
  ...
  const handleStagingConfirm = async (links: ImportManualLinks) => {
      ...
      try {
          ...
          console.error(e);
          showToast({ message: t('msg_import_error'), type: 'error' });
          setImportState('staging');
      }
  };
  ...
  const handleExportClick = async () => {
      if (exportState === 'idle') {
          ...
          try {
              ...
              showToast({ message: t('msg_export_prep_fail'), type: 'error' });
              setExportState('idle');
          }
      } 
          ...
          try {
              ...
              if (navigator.canShare && navigator.canShare({ files: [file] })) {
                  ...
                  showToast({ message: t('msg_share_success'), type: 'success' });
                  return;
              }
          } catch (e) {
          ...
          showToast({ message: t('msg_download_start'), type: 'success' });
      }
  };
      ...
      >
        {importState === 'importing' ? (
             <div className="flex flex-col items-center justify-center h-full gap-4">
                 ...
                 <span className="text-white font-bold">{t('state_importing')}</span>
                 <span className="text-xs text-slate-500">{t('warn_keep_open')}</span>
             </div>
        ) : (
            <>
                ...
                <div className="p-6 space-y-6 flex-1 overflow-y-auto">
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        >
                            ...
                            <span>{importState === 'analyzing' ? t('state_analyzing') : t('btn_import')}</span>
                            {importState !== 'analyzing' && <span className="text-[10px] font-normal opacity-80">{t('btn_import_sub')}</span>}
                        </button>
                        ...
                        >
                            ...
                            <span>
                                {exportState === 'idle' && t('btn_export_idle')}
                                {exportState === 'processing' && t('btn_export_processing')}
                                {exportState === 'ready' && t('btn_export_ready')}
                            </span>
                            
                            {exportState === 'idle' && <span className="text-[10px] font-normal text-slate-500">{t('btn_export_sub')}</span>}
                        </button>
                    </div>
                </div>
