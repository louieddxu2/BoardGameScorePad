    ...
    reader.onload = async (ev) => {
      try {
        ...
        if (!parsed.games && !parsed.plays && !parsed.locations) {
             alert("格式錯誤：這似乎不是 BG Stats 的備份檔");
             setImportState('file_selection');
             return;
        }
  ...
  const handleExportClick = async () => {
      if (exportState === 'idle') {
          ...
          try {
              ...
              showToast({ message: "匯出準備失敗", type: 'error' });
              setExportState('idle');
          }
      } 
          ...
          try {
              ...
              if (navigator.canShare && navigator.canShare({ files: [file] })) {
                  ...
                  showToast({ message: "分享成功", type: 'success' });
                  return;
              }
          } catch (e) {
      ...
      >
        {importState === 'importing' ? (
             <div className="flex flex-col items-center justify-center h-full gap-4">
                 ...
                 <span className="text-white font-bold">正在匯入資料...</span>
                 <span className="text-xs text-slate-500">請勿關閉視窗</span>
             </div>
        ) : (
            <>
                ...
                <div className="p-6 space-y-6 flex-1 overflow-y-auto">
                    <div className="bg-indigo-900/20 border border-indigo-500/30 rounded-xl p-4 text-sm text-indigo-200 leading-relaxed">
                        <p className="flex items-start gap-2">
                            ...
                            此功能可讓您在 Board Game Stats (BG Stats) 與本 App 之間轉移資料。
                        </p>
                    </div>
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        >
                            ...
                            <span>{importState === 'analyzing' ? "正在分析檔案..." : "匯入 BG Stats 紀錄"}</span>
                            {importState !== 'analyzing' && <span className="text-[10px] font-normal opacity-80">讀取 .json 備份檔</span>}
                        </button>
                        ...
                        >
                            ...
                            <span>
                                {exportState === 'idle' && "匯出為 BG Stats 格式"}
                                {exportState === 'processing' && "正在準備檔案..."}
                                {exportState === 'ready' && "下載 / 分享檔案"}
                            </span>
                            
                            {exportState === 'idle' && <span className="text-[10px] font-normal text-slate-500">產生 .json 檔案</span>}
                        </button>
                    </div>
                </div>
