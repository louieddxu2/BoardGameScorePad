...
const SCORING_MODES: { value: ScoringRule, label: string }[] = [
    { value: 'HIGHEST_WINS', label: '最高分贏' },
    { value: 'LOWEST_WINS', label: '最低分贏' },
    { value: 'COOP', label: '合作模式' },
    { value: 'COMPETITIVE_NO_SCORE', label: '競爭(無勝負)' },
    { value: 'COOP_NO_SCORE', label: '合作(無勝負)' },
];
...
const StartGamePanel = React.forwardRef<HTMLDivElement, StartGamePanelProps>(({ 
  ...
  const currentModeLabel = SCORING_MODES.find(m => m.value === scoringRule)?.label || '規則';
  ...
  const renderItem = (option: GameOption, isDocked: boolean = false) => {
      ...
      if (option.bestPlayers && option.bestPlayers.length > 0) {
          metaParts.push(`${option.bestPlayers.join(', ')}人`);
      } else if (option.minPlayers) {
          if (option.maxPlayers && option.maxPlayers !== option.minPlayers) {
              metaParts.push(`${option.minPlayers}-${option.maxPlayers}人`);
          } else {
              metaParts.push(`${option.minPlayers}人`);
          }
      } else if (option.defaultPlayerCount) {
          metaParts.push(`${option.defaultPlayerCount}人`);
      }

      if (option.playingTime) {
          metaParts.push(`${option.playingTime}分`);
      }

      if (option.complexity && option.complexity > 0) {
          metaParts.push(`重度 ${Number(option.complexity).toFixed(1)}`);
      }
                   ...
                   <div className="flex items-stretch w-full h-full gap-3">
                            ...
                            <div className="text-[10px] opacity-60 flex items-center gap-1 h-4">
                                {isVirtual ? (
                                    ...
                                    <>
                                        ...
                                        <span className="truncate">{metaString || '未設定'}</span>
                                    </>
                                )}
                            </div>
                       </div>
                       ...
                       {isSimple && (
                            <button
                                ...
                                `}
                                title={option.isPinned ? "取消釘選" : "建立並釘選此簡易計分板"}
                            >
                                <Pin size={12} className={option.isPinned ? "fill-current" : ""} />
                                <span className="text-[9px] leading-none scale-90 origin-center font-bold">簡易</span>
                            </button>
                       )}
                   </div>
               </button>
