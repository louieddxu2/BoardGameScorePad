...
export const useGameLauncher = ({
  ...
  const handlePanelStart = async (option: GameOption, playerCount: number, location: string) => {
    ...
    try {
      ...
      // 3. Launch Session
      // We pass the template with the *current* playerCount to ensure the session initializes correctly
      const templateWithPrefs = { ...templateToStart, lastPlayerCount: playerCount };
      onTemplateSelect(templateWithPrefs);
      
    } catch (e) {
      console.error("Failed to start game", e);
      // Fallback launch even if prefs fail
      onTemplateSelect(templateToStart);
    }
  };
