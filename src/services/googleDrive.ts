...
class GoogleDriveService {
    ...
    public async emptyTrash(type?: CloudResourceType): Promise<void> {
        if (!this.isAuthorized) await this.signIn();
        ...
        if (type) {
            const trashId = await this.getTrashFolder(type);
            idsToCheck.push(trashId);
        } else {
            // Default: Check all if not specified (backward compatibility)
            ...
            idsToCheck = Array.from(new Set([trashTempId, trashActiveId, trashHistoryId]));
        }
    ...
    public async getFileContent(folderId: string, filename: string = 'data.json'): Promise<any> {
        ...
        if (!file) throw new Error(`Not found in backup: ${filename}`);
