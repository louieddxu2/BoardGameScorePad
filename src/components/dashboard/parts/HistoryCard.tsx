...
import { Crown, Calendar, Trash2 } from 'lucide-react';
    ...
    >
      ...
      <div className="flex justify-between items-start mb-3">
        <div className="flex flex-col gap-1">
            ...
            <h3 className="text-lg font-bold text-white leading-tight">{record.gameName}</h3>
        </div>
        
        <button 
            onClick={(e) => { e.stopPropagation(); if(record.id) onDelete(record.id); }}
            className="p-2 text-slate-600 hover:text-red-400 hover:bg-slate-700/50 rounded-lg transition-colors -mr-2 -mt-2"
        >
            <Trash2 size={16} />
        </button>
      </div>
