            ...
            <div className="modal-backdrop" onClick={onClose}>
                ...
                >
                    ...
                    <div className="modal-header">
                        ...
                        <button onClick={onClose} className="text-txt-muted hover:text-txt-primary shrink-0 p-1"><X size={20} /></button>
                    </div>
                    ...
                    <div className="flex-1">
                        {previewSession ? (
                            ...
                            <div className="grid grid-cols-3 grid-rows-[auto_auto_1fr] h-auto max-h-[70vh]">
                                    ...
                                    <div className="grid grid-cols-2 gap-x-2 gap-y-1">
                                        {previewSession.players.slice(0, 4).map(p => (
                                            <div key={p.id} className="flex justify-between items-center text-xs">
                                                <span className="text-txt-secondary truncate max-w-[60%] font-medium">{p.name}</span>
                                                <span className="font-mono font-bold text-brand-primary">{p.totalScore}</span>
                                            </div>
                                        ))}
                                    </div>
                                    {previewSession.players.length > 4 && <div className="text-[10px] text-txt-muted mt-1 italic">{t('setup_more_players', { count: previewSession.players.length })}</div>}
                                </div>
                                ...
                                <div className="col-span-1 row-span-1 border-b border-surface-border modal-bg-elevated rounded-none p-0.5">
                                    ...
                                    >
                                        ...
                                        <span className="text-sm font-bold opacity-90 leading-tight whitespace-pre-line text-center">{t('setup_resume_btn')}</span>
                                    </button>
                                </div>
                                ...
                                <div className="col-span-2 row-span-1 p-1.5 border-r border-surface-border flex flex-col justify-center items-center gap-1.5 overflow-visible rounded-bl-2xl">
                                    ...
                                    <div className="flex gap-2 w-full relative z-20">
                                        ...
                                        <div className="flex-1 bg-surface-bg-alt rounded-lg p-0.5 border border-surface-border flex items-center justify-center relative min-w-0">
                                            <input
                                                type="time"
                                                value={startTimeStr}
                                        ...
                                        <div className="flex-1 modal-bg-elevated p-0.5 flex items-center relative min-w-0" title={t('setup_rule')}>
                                            ...
                                            >
                                                ...
                                                <ChevronDown size={12} className="text-txt-muted shrink-0 ml-0.5" />
                                            </button>
                                                ...
                                                <div className="absolute bottom-full left-0 right-0 mb-1 modal-container shadow-xl z-[70] overflow-hidden min-w-[140px] -right-4 animate-in fade-in zoom-in-95 duration-200">
                                                    {reversedOptions.map(opt => (
                                                        <button
                                                            ...
                                                            className={`w-full text-left px-3 py-2.5 text-xs font-bold border-b border-surface-border last:border-0 hover:bg-surface-bg-alt flex items-center justify-between ${scoringRule === opt.value ? 'text-brand-primary bg-brand-primary/10' : 'text-txt-secondary'}`}
                                                        >
                                                            {opt.label}
                                    ...
                                    <div className="flex items-center justify-center gap-2 w-full z-10">
                                        ...
                                        <div className="min-w-[2rem] flex flex-col items-center justify-center">
                                            ...
                                            <span className="text-[9px] text-txt-muted uppercase font-bold">{t('player_unit')}</span>
                                        </div>
                                ...
                                <div className="col-span-1 row-span-1 p-1.5 flex flex-col items-center justify-center modal-bg-recessed gap-1.5 rounded-br-2xl">
                                    ...
                                    <button
                                        ...
                                        className="w-full flex-1 max-h-12 bg-brand-secondary hover:filter hover:brightness-110 active:brightness-90 text-white rounded-lg shadow-md active:scale-95 transition-all flex items-center justify-center"
                                    >
                            ...
                            <div className="p-3 flex flex-col items-center gap-3 rounded-b-2xl">
                                    ...
                                    <div className="flex flex-col gap-1 relative">
                                        ...
                                        <div className="modal-bg-elevated px-2 py-1 w-full border border-surface-border rounded-lg flex items-center relative hover:border-brand-primary h-[38px] transition-colors">
                                            ...
                                            >
                                                ...
                                                <ChevronDown size={12} className="text-txt-muted pointer-events-none shrink-0" />
                                            </button>
                                                ...
                                                <div className="absolute bottom-full left-0 right-0 mb-1 modal-container shadow-xl z-[70] overflow-hidden min-w-[160px] -right-2 animate-in fade-in zoom-in-95 duration-200">
                                                    {reversedOptions.map(opt => (
                                                        <button
                                                            ...
                                                            className={`w-full text-left px-3 py-3 text-xs font-bold border-b border-surface-border last:border-0 hover:bg-surface-bg-alt flex items-center justify-between ${scoringRule === opt.value ? 'text-brand-primary bg-brand-primary/10' : 'text-txt-secondary'}`}
                                                        >
                                                            {opt.label}
                                ...
                                <button
                                    onClick={handleStartClick}
                                    className="w-full bg-brand-secondary hover:filter hover:brightness-110 text-white font-bold py-2.5 rounded-xl flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-all z-10"
                                >
