            ...
            {selectedItem ? (
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-200">
                    <div className="flex items-center gap-3 mb-4 pb-4 border-b border-slate-700/50">
                        <div className="p-3 bg-indigo-900/40 border border-indigo-500/30 rounded-xl text-indigo-400 shadow-sm"><Icon size={24} /></div>
                        <div>
                    ...
                    {/* BGG Info Card */}
                    {bggInfo && (
                        <div className="bg-indigo-950/20 border border-indigo-500/20 rounded-xl p-3 flex flex-col gap-3 animate-in fade-in slide-in-from-top-2 shadow-inner">
                            <div className="w-full">
                                <div className="flex items-start justify-between mb-2">
                                    <h4 className="font-bold text-indigo-400 text-sm truncate">{bggInfo.name}</h4>
                                    <a
                                        href={`https://boardgamegeek.com/boardgame/${bggInfo.id}`}
                                        target="_blank"
                                        rel="noreferrer"
                                        className="text-indigo-400 hover:text-indigo-300 p-1 bg-indigo-900/40 rounded transition-colors"
                                        title="Open in BGG"
                                    >
...
export const DataList = ({ title, table, icon: Icon, isBGG = false }: { title: string, table: any, icon: any, isBGG?: boolean }) => {
    ...
    if (!data) return <div className="p-4 text-slate-500">{tCommon('loading')}</div>;
                ...
                <div className="p-2 space-y-1">
                    {data.map((item: any) => (
                        <button
                            ...
                            className={`w-full text-left p-2 rounded-lg text-xs transition-all flex justify-between items-center ${selectedId === item.id ? 'bg-emerald-600 text-white shadow-md' : 'bg-slate-800 text-slate-300 hover:bg-slate-750 hover:text-slate-100'}`}
                        >
