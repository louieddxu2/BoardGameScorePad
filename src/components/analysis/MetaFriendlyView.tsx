...
const CollapsibleSection = ({ icon, title, count, confidence, children }: { icon: React.ReactNode, title: string, count: number, confidence?: number, children?: React.ReactNode }) => {
    ...
    return (
        <div className="border border-surface-border rounded-lg overflow-hidden bg-app-bg-deep/30">
            <button
                onClick={() => setIsOpen(!isOpen)}
                className="w-full flex items-center justify-between p-2 hover:bg-surface-hover transition-colors"
            >
                <div className="flex items-center gap-2">
                    {isOpen ? <ChevronDown size={14} className="text-txt-muted" /> : <ChevronRight size={14} className="text-txt-muted" />}
                    <div className="flex items-center gap-1.5">
                        {icon}
                        <span className="text-xs font-bold text-txt-primary">{title}</span>
                    </div>
                </div>
                <div className="flex items-center gap-2">
                    {confidence !== undefined && (
                        <span className="text-[9px] font-mono text-status-info bg-status-info/10 px-1.5 py-0.5 rounded border border-status-info/20 flex items-center gap-1" title={t('confidence_hint')}>
                            <Activity size={10} />
                            {confidence.toFixed(2)}
                        </span>
                    )}
                    <span className="text-[10px] text-txt-muted bg-surface-bg px-1.5 rounded">{count}</span>
                </div>
            </button>
            {isOpen && (
                <div className="p-2 border-t border-surface-border bg-black/20">
                    {children}
                </div>
            )}
        </div>
...
const MetaFriendlyView = ({ meta }: { meta: any }) => {
    ...
    return (
        <div className="space-y-1">
            {renderCategory('players', <Users size={12} className="text-brand-secondary" />, t('rel_players'), (id, { entity }) => (
                <span className="text-txt-primary">{entity?.name || id}</span>
            ))}

            {renderCategory('locations', <MapPin size={12} className="text-status-danger" />, t('rel_locations'), (id, { entity }) => (
                <span className="text-txt-primary">{entity?.name || id}</span>
            ))}

            {renderCategory('games', <LayoutGrid size={12} className="text-brand-primary" />, t('rel_games'), (id, { entity }) => (
                <span className="text-txt-primary">{entity?.name || id}</span>
            ))}

            {renderCategory('playerCounts', <Hash size={12} className="text-status-warning" />, t('rel_player_counts'), (id, { entity }) => (
                <span className="text-txt-primary">{entity?.name || id} {t('unit_player')}</span>
            ))}

            {renderCategory('gameModes', <Trophy size={12} className="text-status-warning" />, t('rel_modes'), (id, { entity }) => (
                <span className="text-txt-primary">{entity?.name || id}</span>
            ))}

            {renderCategory('colors', <Palette size={12} className="text-brand-secondary" />, t('rel_colors'), (id) => (
                <div className="flex items-center gap-2">
                    <div className="w-3 h-3 rounded-full border border-white/20" style={{ backgroundColor: id === 'transparent' ? 'transparent' : id }}>
                        {id === 'transparent' && <div className="w-full h-full border border-surface-border rounded-full" />}
                    </div>
                    <span className="text-txt-muted">{id}</span>
                </div>
            ))}

            {renderCategory('weekdays', <Calendar size={12} className="text-status-info" />, t('rel_weekdays'), (id, { entity }) => {
                const dayIdx = parseInt(entity?.name || '0', 10);
                return <span className="text-txt-primary">{getWeekdayName(dayIdx)}</span>;
            })}
