...
const MultiplayerPlayerClaimModal: React.FC<MultiplayerPlayerClaimModalProps> = ({ isOpen, players, onConfirm, onClose, variant = 'join', initialSelectedIds = [] }) => {
  ...
  const titleKey = isManage
    ? 'multiplayer_manage_claim_title'
    : isClaim
      ? 'multiplayer_claim_title'
      : 'multiplayer_join_title';
  const descriptionKey = isManage
    ? 'multiplayer_manage_claim_desc'
    : isClaim
      ? 'multiplayer_claim_desc'
      : 'multiplayer_join_desc';
  const confirmKey = isManage
    ? 'multiplayer_manage_claim_confirm'
    : isClaim
      ? 'multiplayer_claim_confirm'
      : 'multiplayer_join_confirm';
  ...
  return (
    <div className="modal-backdrop p-4" style={{ zIndex }} onClick={onClose}>
      <section className="modal-container w-full max-w-sm p-5" onClick={(event) => event.stopPropagation()}>
        ...
        <button type="button" disabled={!selectedIds.length && !isManage} onClick={() => onConfirm(selectedIds)} className="w-full mt-5 py-3 rounded-lg bg-brand-primary text-white font-bold disabled:opacity-40 disabled:cursor-not-allowed">
          {t(confirmKey)}
        </button>
      </section>
    </div>
