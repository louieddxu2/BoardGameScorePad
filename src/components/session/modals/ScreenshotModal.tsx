...
const ScreenshotModal: React.FC<ScreenshotModalProps> = ({
    ...
    useEffect(() => {
        ...
        const generateCurrentMode = async () => {
            ...
            try {
                ...
                showToast({ message: t('photo_msg_capture_fail'), type: 'error' });
            } finally {
                setIsGenerating(false);
            }
        };
    ...
    const handleCopy = async () => {
        ...
        try {
            ...
            showToast({ message: t('photo_msg_copy_success'), type: 'success' });
        } catch (err) {
            console.error(err);
            showToast({ message: t('photo_msg_copy_fail'), type: 'error' });
        }
    };
    ...
    const handleDownload = () => {
        ...
        showToast({ message: t('photo_msg_share_success'), type: 'success' });
    };
        ...
        try {
            ...
            if (navigator.canShare && navigator.canShare({ files: [file] })) {
                await navigator.share({
                    files: [file],
                    title: template.name,
                    text: t('photo_share_text', { name: template.name })
                });
            } else {
                showToast({ message: t('photo_msg_share_no_support'), type: 'warning' });
            }
        } catch (err) {
      ...
      */}
            ...
            >
                ...
                <div className="flex-1 min-h-0 bg-app-bg-deep relative flex flex-col z-0 overflow-hidden">
                    <div
                        ref={containerRef}
                        className="flex-1 min-h-0 w-full relative overflow-hidden bg-[radial-gradient(rgb(var(--c-surface-border)_/_0.5)_1px,transparent_1px)] [background-size:16px_16px] touch-none cursor-grab active:cursor-grabbing"
