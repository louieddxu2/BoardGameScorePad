...
const ColumnConfigEditor: React.FC<ColumnConfigEditorProps> = ({ column, onSave, onDelete, onClose }) => {
  ...
  const handleSave = () => {
      ...
      if (activeTab === 'mapping') {
          ...
          if (currentMode === 'product') {
              ...
              // Fix: Respect the user's choice for inputType, default to keypad if undefined
  ...
  const updateMappingRule = (idx: number, field: keyof MappingRule, val: any) => {
      ...
      if (field === 'isLinear' && idx > 0) {
          ...
          updatedRule.unit = val ? 1 : undefined;
          updatedRule.score = val ? 1 : updatedRule.score;
      } else {
          let newValue = val;
  ...
  const setCalculationMode = (mode: CalculationMode) => {
      ...
      // Note: If switching to 'sum-parts', we preserve the current inputType (or default to keypad via handleSave if undefined)
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/95 flex flex-col animate-in slide-in-from-bottom-5" style={{ paddingBottom: visualViewportOffset }}>
      ...
      {!isKeyboardOpen && (
          <footer className="fixed bottom-0 left-0 right-0 p-4 bg-slate-900/80 backdrop-blur-sm border-t border-slate-800 z-20 animate-in slide-in-from-bottom-2 duration-200" style={{ paddingBottom: `calc(1rem + ${visualViewportOffset}px)` }}>
