...
Write-Host "[1/4] Scanning for hardcoded Chinese strings..." -ForegroundColor White
try {
    powershell -ExecutionPolicy Bypass -File "$PSScriptRoot\scan-hardcoded-chinese.ps1"
    if ($LASTEXITCODE -ne 0) { $allPassed = $false; Write-Host "[1/4] FAIL" -ForegroundColor Red }
    else { Write-Host "[1/4] OK" -ForegroundColor Green }
}
catch {
...
Write-Host ""
Write-Host "[2/4] TypeScript type check (tsc)..." -ForegroundColor White
...
if ($tscExitCode -ne 0) {
    ...
    Write-Host "[2/4] FAIL" -ForegroundColor Red
    $allPassed = $false
}
else {
...
# Step 3: Unit tests
Write-Host ""
Write-Host "[3/4] Running unit tests (vitest)..." -ForegroundColor White
...
# Step 4: Token integrity (dark/light CSS variable completeness)
Write-Host ""
Write-Host "[4/4] Token integrity check..." -ForegroundColor White
$tokenOutput = cmd /c "npx vitest run src/utils/token-integrity.test.ts --no-color 2>&1" | Out-String
$tokenExitCode = $LASTEXITCODE

if ($tokenExitCode -ne 0) {
    Write-Host "Token integrity errors found:" -ForegroundColor Yellow
    Write-Host $tokenOutput -ForegroundColor Gray
    Write-Host "[4/4] FAIL" -ForegroundColor Red
    $allPassed = $false
}
else {
    $tokenSummary = $tokenOutput | Select-String "Tests.*passed" | Select-Object -Last 1
    if ($tokenSummary) { Write-Host $tokenSummary.Line.Trim() -ForegroundColor Gray }
