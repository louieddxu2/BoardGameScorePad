...
為了優化 UI 密度與使用者視覺體驗，本專案採取**「沈穩黑曜岩單色背景 ＋ 微縮彩色圖示 ＋ 純數值與單位尾綴」**的超輕量徽章設計，徹底消除彩虹塊的視覺干擾，呈現極致內斂的高奢深色主題質感。

### 1. 標準搜尋結果附註列（已實作項目，由左至右排版）
* 🏆 **最佳人數 (Best Player Count)**：
  * **圖示**：`<Star size={10} className="fill-amber-400/20 text-amber-500/80 mr-0.5 shrink-0" />`
  * **數值與單位**：`{bestPlayers}人` (英文為 `P`)
  * **膠囊樣式**：`bg-surface-bg-alt/80 text-txt-secondary border border-surface-border/30 px-1.5 py-0.5 rounded text-[9px] leading-none shrink-0 font-medium`
* 👥 **支援人數 (Supported Player Count)**：
  * **圖示**：`<Users size={10} className="text-sky-500/80 mr-0.5 shrink-0" />`
  * **數值與單位**：`{min}-{max}人` (英文為 `P`)
  * **膠囊樣式**：`bg-surface-bg-alt/80 text-txt-secondary border border-surface-border/30 px-1.5 py-0.5 rounded text-[9px] leading-none shrink-0 font-medium`
* 🧠 **遊戲重度 (Weight / Complexity)**：
  * **圖示**：`<Brain size={10} className="text-purple-500/80 mr-0.5 shrink-0" />`
  * **數值**：`{complexity}` (保留一位小數，無尾綴)
  * **膠囊樣式**：`bg-surface-bg-alt/80 text-txt-secondary border border-surface-border/30 px-1.5 py-0.5 rounded text-[9px] leading-none shrink-0 font-medium`
* 📅 **發行年份 (Year Published)**：
  * **圖示**：`<Calendar size={10} className="text-emerald-500/80 mr-0.5 shrink-0" />`
  * **數值與單位**：`{year}年` (英文為 `{year}` 無尾綴)
  * **膠囊樣式**：`bg-surface-bg-alt/80 text-txt-secondary border border-surface-border/30 px-1.5 py-0.5 rounded text-[9px] leading-none shrink-0 font-medium`

### 2. 篩選搜尋下的特有標籤提示（🚨 待實作與設計項目）
當進階篩選模式處於作用中時，系統需要在匹配的遊戲上呈現以下突出的特定亮點徽章，此部分目前**尚未實作，為下一步核心開發目標**：
* 🤝 **合作遊戲 (Cooperative)**：符合 `cooperative === true` 且匹配時，顯示專屬特色合作徽章，伴隨溫和的呼吸效果。
* 🎪 **小桌面 (Small Table)**：符合 `複雜度 <= 2.2` 且 `時長 <= 45分` 且匹配時，顯示 `小品` / `小桌` 特色膠囊。
* **狀態與排版**：需在後續迭代中設計其在篩選搜尋面板中的相對位置、展開時的過濾呼應、以及中英文雙語標籤的整合字典。
